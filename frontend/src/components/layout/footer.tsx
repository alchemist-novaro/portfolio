...
export default function Footer() {
  ...
  const footerLinks: FooterLinks = {
    ...
    resources: [
      ...(isAuthenticated
        ? routes.authorized.filter((route) =>
            route.type.includes("f-resources")
          )
        : routes.unauthorized.filter((route) =>
            route.type.includes("f-resources")
          )),
      ...routes.default.filter((route) => route.type.includes("f-resources")),
    ],
    legal: [...routes.default.filter((route) => route.type.includes("f-legal"))],
  };
        ...
        >
          ...
          <motion.div variants={itemVariants} className="space-y-4">
            ...
            <p className="text-secondary-foreground/80 text-sm leading-relaxed">
              ...
              vision, generative AI, chatbots, and AI agents — building
              centralized and decentralized AI systems that power scalable
              applications and business transformation
            </p>
              ...
              {footerLinks.services.map((link) => (
                <li key={link.name}>
                  <motion.div
                    ...
                    data-testid={`footer-service-${link.name
                      .toLowerCase()
                      .replace(" ", "-")}`}
                  >
                    {link.name}
                ...
                <li key={link.name}>
                  <Link to={link.path}>
                    <motion.div
                      whileHover={{ x: 4 }}
                      className="text-secondary-foreground/80 hover:text-secondary-foreground transition-colors duration-200 text-sm"
                      data-testid={`footer-resource-${link.name
                        .toLowerCase()
                        .replace(" ", "-")}`}
                    >
                      {link.name}
